...
class Kepler():
    ...
    def mag_data(self):
        ...
        for line in kep_file:
            line = line.split()
            if line[7] != 'nan':
                ap_corr_mag.append(-2.5*log10(eval(line[7]))) 
        avg_all_mag = average(ap_corr_mag)
        C = avg_Kmag - avg_all_mag
        ap_corr_mag_withC = [mag + C  for mag in ap_corr_mag] #mag after adding C (absolute mag in K-Band)
        kep_file.close()
    ...
    def flux_data(self):
        ...
        for line in kep_file:
            line = line.split()
            if line[7] != 'nan':
                ap_corr_flux.append(eval(line[7])) 
        kep_file.close()
        return ap_corr_flux
    ...
    def magErr_data(self):
        ...
        for line in kep_file:
            ...
            if line[7] != 'nan':
                ap_corr_mag.append(-2.5*log10(eval(line[7])))
                ap_corr_err.append(2.5*log10((eval(line[7]) + eval(line[8]))/eval(line[7])))
        avg_all_mag = average(ap_corr_mag)
        C = avg_Kmag - avg_all_mag
        ap_corr_err_withC = [err  for err in ap_corr_err] #mag err after adding C (absolute mag err in K-Band)
        kep_file.close()
    ...
    def fluxErr_data(self):
        ...
        for line in kep_file:
            line = line.split()
            if line[7] != 'nan':
                ap_corr_fluxErr.append(eval(line[8])) 
        kep_file.close()
...
def display_help():
  ...
  print "   ################################################################################################"
  print "   #      kplrlca (kepler_04.py) is a program for analyzing the light curves of Eclipsing"
  ...
  print "   #       This program (kplrlca) takes a light curve (txt format) with data for all the"
  ...
  print "   ################################################################################################"
  print "USAGE: kplrlca [OPTIONS] [FILES]"
  print ""
...
def display_usage():
  print ''
  print "USAGE: kplrlca [OPTIONS] [FILES]"
  print "Type \'kplrlca -h\' for detailed help."
  print ''
